...
Constant *CreateWords(Module* mod, string str)
{
	...
	if (my_name)
	{
		...
		const_ptr_indices.push_back(const_int);
		Constant* const_ptr = ConstantExpr::getGetElementPtr(ArrayTy, my_global, const_ptr_indices);
		return const_ptr;
	}
	...
	const_ptr_indices.push_back(const_int);
	const_ptr_indices.push_back(const_int);
	Constant* const_ptr = ConstantExpr::getGetElementPtr(ArrayTy, global_name, const_ptr_indices);
...
{
	llvm::outs() << "in Log func\n";;
	func_record = RecFuncInfo(mod);
	for (Module::iterator it_MM = mod->begin(); it_MM != mod->end(); it_MM++)
	{
		Function *MyFn = &(*it_MM);
		//Leave alone inline callers 
		AttributeList attrList = MyFn->getAttributes();
		int attr_num = attrList.getNumAttrSets();
		//outs() << attr_num << "\n";
		bool attr_flag = false;
		for (int i=0; i<attr_num; i++)
		{
			AttributeSet attr = attrList.getAttributes(i);
			if (attr.hasAttribute(Attribute::InlineHint) == true)
			{
				attr_flag = true;
				break;
			}
...
void CreatePairRec(CallInst *mycall, char *name, Module* mod)
{
	...
	if (myvalue->getType()->isPointerTy() == true)
	{
		...
		AllocaInst* alloc_data = new AllocaInst(PtrTy, 0, "", MyIn);
