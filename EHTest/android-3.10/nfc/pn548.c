...
#define DEBUG
#ifdef DEBUG
#define FUNC_LOG()  pr_err("FUNC_LOG: [%d][%s()]\n", __LINE__, __FUNCTION__)
#endif
...
static void pn548_disable_irq(struct pn548_dev *pn548_dev)
{
	...
	if (pn548_dev->irq_enabled) {
		...
		pr_err(PN548_DRV_NAME ":%s disable IRQ\n", __func__);
	} else {
...
static void pn548_enable_irq(struct pn548_dev *pn548_dev)
{
	...
	if (!pn548_dev->irq_enabled) {
		...
		pr_err(PN548_DRV_NAME ":%s enable IRQ\n", __func__);
	} else {
					...
					size_t count, loff_t *offset)
{
	...
	{
		pr_err(PN548_DRV_NAME ": %d\n", pn548_dev->irq_gpio);
		ret = wait_event_interruptible(pn548_dev->read_wq,
					gpio_get_value(pn548_dev->irq_gpio));
		pr_err(PN548_DRV_NAME ":gpio_get_value %d\n",gpio_get_value(pn548_dev->irq_gpio));
		if (ret == 0)
			isFirstPacket = false;
	}
	FUNC_LOG();
	...
	case pn548_SET_PWR:
		if (arg == 2) {
			...
			if (sPowerState == NFC_POWER_OFF) {
				...
				pr_info(PN548_DRV_NAME ":%s %d\n", __func__, sPowerState);
